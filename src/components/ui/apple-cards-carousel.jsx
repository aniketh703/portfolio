...
export const Carousel = ({ items, initialScroll = 0 }) => {
  ...
  const handleCardClose = (index) => {
    if (carouselRef.current) {
      const container = carouselRef.current.querySelector('.flex.flex-row');
...
export const Card = ({ card, index, layout = false }) => {
  ...
  // intercepts wheel/touch independently of CSS), so the page behind this
  // modal would still scroll without also calling lenis.stop()/start().
  useEffect(() => {
