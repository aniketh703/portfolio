...
const Archive = ({ projects, onSelect }) => {
  const [hoveredId, setHoveredId] = useState(null);
  const [cursorPos, setCursorPos] = useState({ x: 0, y: 0 });
  const archiveList = [...projects, ...projects, ...projects].map((p, i) => ({ ...p, uniqueId: i }));
  ...
  return (
    <section className="min-h-screen bg-stone-50 pt-32 pb-20 px-4 md:px-12 relative">
      ...
      <div className="w-full">
        ...
        {archiveList.map((project, index) => (
          <div key={project.uniqueId} onClick={() => onSelect(project)} onMouseEnter={() => setHoveredId(index)} onMouseLeave={() => setHoveredId(null)} className="group grid grid-cols-12 gap-4 items-center py-6 border-t border-stone-300 hover:bg-stone-900 hover:text-stone-50 transition-colors duration-200 cursor-pointer px-4">
